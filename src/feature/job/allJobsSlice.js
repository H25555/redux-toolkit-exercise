...
export const getAllJobs = createAsyncThunk('allJobs/getJobs',
    async (_, thunkAPI) => {
        ...
        try {
            const resp = await axios.get(`https://jobify-prod.herokuapp.com/api/v1/toolkit${url}`,
                {
                    headers: {
...
export const showStats = createAsyncThunk('allJobs/showStats', async (_, thunkAPI) => {
    try {
        const resp = await axios.get(`https://jobify-prod.herokuapp.com/api/v1/toolkit/jobs/stats`,
            {
                headers: {
                    authorization: `Bearer ${thunkAPI.getState().user.user.token}`
                }
